...
class NormalizedLocatedObjects(UserList):
    def __init__(self, objects: Optional[Iterable[NormalizedLocatedObject]] = None):
        super().__init__(objects)

    def subset(self, indices: List[int]) -> "NormalizedLocatedObjects":
        """
        ...
        return NormalizedLocatedObjects((obj for obj in self if obj.get_index() in indices))

    def remove(self, item: Union[NormalizedLocatedObject, List[int]]) -> None:
        """
        ...
        """
        # List implementation
        if isinstance(item, NormalizedLocatedObject):
    ...
    def find(self, index: Union[int, str]) -> Optional[NormalizedLocatedObject]:
        """
    ...
    @classmethod
    def from_report(cls, report: Report, prefix: str = "Object.") -> "NormalizedLocatedObjects":
        """
        ...
        result: NormalizedLocatedObjects = NormalizedLocatedObjects()
        ...
        for group, group_fields in groups.items():
            ...
            try:
                if (report.has_value(f"{group}.{constants.KEY_X}") and
                    ...
                    x: float = float(report.get_real_value(f"{group}.{constants.KEY_X}"))
                    y: float = float(report.get_real_value(f"{group}.{constants.KEY_Y}"))
                    width: float = float(report.get_real_value(f"{group}.{constants.KEY_WIDTH}"))
                    height: float = float(report.get_real_value(f"{group}.{constants.KEY_HEIGHT}"))
                    obj: NormalizedLocatedObject = NormalizedLocatedObject(x, y, width, height, **meta)
                    result.append(obj)
    ...
    def __iter__(self) -> Iterator[NormalizedLocatedObject]:
